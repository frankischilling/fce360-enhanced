 ...
 */
 ...
 private:
	 ...
	 static bool CompareRomItems(const rom_item& a, const rom_item& b) {
		 // Convert to lowercase for case-insensitive comparison
		 std::wstring aLower = a.affichage;
		 std::wstring bLower = b.affichage;
		 for(size_t i = 0; i < aLower.length(); i++)
			 aLower[i] = towlower(aLower[i]);
		 for(size_t i = 0; i < bLower.length(); i++)
			 bLower[i] = towlower(bLower[i]);
 ...
 private:
	 ...
	 void ApplySearchFilter()
	 {
		 ...
		 if (m_searchFilter.empty())
		 {
			 ...
			 // Add all ROMs from the full list (items can appear in both Recent/Favorites and general list)
	 ...
	 HRESULT ScanDir()
	 {
			 ...
			 {
				 {
					 if(!(FileInformation.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY ))
					 {
						 ...
						 char filenameLower[260];
						 strncpy(filenameLower, FileInformation.cFileName, sizeof(filenameLower) - 1);
						 filenameLower[sizeof(filenameLower) - 1] = '\0';
						 for(int i = 0; filenameLower[i]; i++)
							 filenameLower[i] = tolower(filenameLower[i]);
						 
						 // Check extension at the end of filename (more precise than strstr)
						 int len = strlen(filenameLower);
						 bool hasValidExtension = false;
						 
						 if(len >= 4 && strcmp(filenameLower + len - 4, ".nes") == 0)
							 hasValidExtension = true;
						 else if(len >= 4 && strcmp(filenameLower + len - 4, ".fds") == 0)
							 hasValidExtension = true;
						 else if(len >= 4 && strcmp(filenameLower + len - 4, ".zip") == 0)
							 hasValidExtension = true;
		 ...
		 // Sort the ROM list alphabetically by filename (case-insensitive)
		 // This ensures NES and FDS files are intermingled properly
		 std::sort(m_rom_list_full.begin(), m_rom_list_full.end(), CompareRomItems);
