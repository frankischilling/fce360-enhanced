...
// Backwards-compatible overload. Origin assumed at (0,0) relative to dest.
void DrawTextTransScaled(uint8 *dest, uint32 width, uint8 *textmsg, uint8 fgcolor, float scaleX, float scaleY)
{
	DrawTextTransScaled(dest, width, 0, 0, textmsg, fgcolor, scaleX, scaleY);
}
...
void DrawTextTransScaled(uint8 *dest, uint32 width, int startX, int startY, uint8 *textmsg, uint8 fgcolor, float scaleX, float scaleY)
{
	...
	const int maxX = (int)width;
	const int maxY = 240; // Overlay height is fixed at 240 pixels
	const int bufSize = maxX * maxY;
	...
	for (; *textmsg; ++textmsg)
	{
		...
		for (int srcY = 0; srcY < 7; ++srcY)
		{
			...
			int globalYStart = startY + dstYStart;
			int globalYEnd = startY + dstYEnd;

			if (globalYStart >= maxY) break;
			if (globalYEnd <= 0) continue;
			...
			for (int srcX = 0; srcX < srcWid; ++srcX)
			{
				if ((d >> (7 - srcX)) & 1)
				{
					...
					int globalXStart = startX + dstXStart;
					int globalXEnd = startX + dstXEnd;

					if (globalXStart >= maxX) break;
					if (globalXEnd <= 0) continue;

					for (int dy = globalYStart; dy < globalYEnd && dy < maxY; ++dy)
					{
						if (dy < 0) continue;

						int rowOffset = dy * maxX;
						if (rowOffset < 0 || rowOffset >= bufSize) continue;

						for (int dx = globalXStart; dx < globalXEnd && dx < maxX; ++dx)
						{
							if (dx < 0) continue;
							int offs = rowOffset + dx;
